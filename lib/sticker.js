...
async function stickerAnimado(img, url, packname, author) {
  ...
  try {
    ...
    const duracionReal = parseFloat(stdout.trim()) || 6
    const duracion = Math.min(duracionReal, 6) // máximo permitido en WhatsApp: 6 segundos

    // FPS dinámico según duración para mantenerse bajo 500KB
    const fps = duracion <= 2 ? 20 : duracion <= 4 ? 15 : 12

    const vf = `fps=${fps},scale='if(gt(iw,ih),512,-2)':'if(gt(ih,iw),512,-2)',pad=512:512:(512-iw)/2:(512-ih)/2:color=#00000000,format=rgba`
    ...
    // Si supera 500KB, recomprimir con menor calidad
    if (buf.length > 500 * 1024) {
      console.log('[stickerAnimado] Superó 500KB, recomprimiendo...')
      const tmpOut2 = path.join(tmp, `anim_${base}_v2.webp`)

      const fpsBajo = Math.max(fps - 4, 8)
      const vf2 = `fps=${fpsBajo},scale='if(gt(iw,ih),512,-2)':'if(gt(ih,iw),512,-2)',pad=512:512:(512-iw)/2:(512-ih)/2:color=#00000000,format=rgba`
      ...
      const buf2 = await fs.promises.readFile(tmpOut2)
...
async function stickerEstatico(img, url, packname, author) {
  ...
  try {
    ...
    const vf = `scale='if(gt(iw,ih),512,-2)':'if(gt(ih,iw),512,-2)',pad=512:512:(512-iw)/2:(512-ih)/2:color=#00000000,format=rgba`
