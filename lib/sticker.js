...
async function stickerAnimadoRaw(buffer, packname, author) {
  ...
  try {
    ...
    try {
      await run(
        `ffmpeg -y -nostdin -threads 1 -i "${tmpIn}" -t ${dur} -c copy -avoid_negative_ts make_zero "${tmpCut}"`,
        30000
      )
      ...
      try {
        await run(
          `ffmpeg -y -nostdin -threads 1 -i "${tmpIn}" -t ${dur} -an "${tmpCut}"`,
          60000
        )
        const cutBuf = await fs.promises.readFile(tmpCut).catch(() => null)
        if (cutBuf && cutBuf.length >= 100) cutSource = tmpCut
    ...
    if (!buf) throw new Error('🩸 Denji dice: no pude convertir ese video en sticker, intenta con uno más corto o liviano.')
    return buf
  } finally {
    fs.promises.unlink(tmpIn).catch(() => {})
