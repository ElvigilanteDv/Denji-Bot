...
async function stickerAnimado(img, url, packname, author) {
  ...
  try {
    ...
    // Comando directo igual que la lógica robada pero con calidad subida
    const cmd = `ffmpeg -y -i "${tmpIn}" -ss 0 -t 6 -an -vcodec libwebp -loop 0 -vsync 0 -vf "fps=12,scale=480:480:force_original_aspect_ratio=decrease,format=rgba,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=#00000000" -qscale 60 -preset picture "${tmpOut}"`
...
async function sticker(img, url, packname, author, categories = [''], extra = {}) {
  ...
  // Detectar si es video/gif para usar el animado
