...
async function stickerAnimado(buffer, packname, author) {
  ...
  try {
    ...
    if (durReal > MAX_DURATION) {
      throw new Error(`🩸 Denji dice: ese video está muy largo (${durReal.toFixed(1)}s). Recórtalo a máximo ${MAX_DURATION} segundos y mándalo de nuevo.`)
    }
    ...
    try {
      await run(
        `ffmpeg -y -nostdin -threads 0 -i "${tmpIn}" -t ${dur} -c copy -avoid_negative_ts make_zero "${tmpCut}"`,
        30000
      )
      ...
      try {
        await run(
          `ffmpeg -y -nostdin -threads 0 -i "${tmpIn}" -t ${dur} -an "${tmpCut}"`,
          45000
        )
        const cutBuf = await fs.promises.readFile(tmpCut).catch(() => null)
        if (cutBuf && cutBuf.length >= 100) cutSource = tmpCut
    ...
    for (let i = 0; i < attempts.length; i++) {
      ...
      try {
        await run(buildCmd(fpsVal, q, c, out), 45000)
...
async function sticker(buffer, opts = {}) {
  ...
  if (buffer.length > MAX_INPUT_SIZE) throw new Error('🩸 Denji dice: ese archivo está muy pesado (máx 50MB).')
