...
conn.ev.on('group-participants.update', async ({ id, participants, action }) => {
  ...
  try {
    ...
    let settings = {}
    try {
      settings = JSON.parse(fs.readFileSync('./json/settings.json', 'utf8'))
    } catch {}
    const chat = settings?.[botNumber]?.[id] || {}
    ...
    for (let user of participants) {
      if (action === 'add') {
        let texto = '🪚「 DENJI BOT — BIENVENIDA SANGRIENTA 」🩸\n\n'
        ...
        await conn.sendMessage(id, { image: { url: pp }, caption: texto, mentions: [user] })

      } else if (action === 'remove') {
        let texto = '🪚「 DENJI BOT — EXPULSADO DEL MATADERO 」🩸\n\n'
